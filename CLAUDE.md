...
This is a Python package using modern tooling (uv for development, meson-python for building) with a modular architecture:

```
src/git_stage_batch/
├── __init__.py          # Package metadata (imports version from _version.py)
├── _version.py.in       # Version template (generated by meson at build time)
...
├── pyproject.toml       # Python package metadata and build backend config
├── scripts/git-stage-batch.in  # Entry point template for system installs
└── docs/git-stage-batch.1      # Man page
```
...
This project uses **meson-python** as the build backend. Development workflow uses **uv** (`uv sync`, `uv run pytest`, `uv build`).

## Dependencies

This project has **zero runtime dependencies** - only stdlib. Keep it that way.

Dev dependencies (pytest, meson-python, ninja) are fine to add if they improve development experience.
