...
def command_suggest_fixup(boundary: str | None = None, reset: bool = False, abort: bool = False, show_last: bool = False) -> None:
    ...
    try:
        ...
        exit_with_error(_("Invalid boundary ref: {}").format(effective_boundary))
    ...
    try:
        ...
        exit_with_error(_("Failed to get commit range {}..HEAD").format(effective_boundary))

    if not rev_list_result.stdout.strip():
        exit_with_error(_("No commits found in range {}..HEAD").format(effective_boundary))
    ...
    if show_last:
        if not state or not state.get("last_shown_commit"):
            print(_("No previous candidate to show."))
            print(_("Run suggest-fixup without --last to find a candidate."))
            sys.exit(1)
        ...
        print(_("Candidate {}: {}").format(iteration, commit_info))
        _show_commit_diff_for_file(candidate_commit, current_lines.path)
        print(_("Run: git commit --fixup={}").format(candidate_commit[:7]))
        return
    ...
    if not candidate_commit:
        if iteration == 1:
            print(_("No commits in range {}..HEAD modified these lines.").format(effective_boundary))
            print(_("The changes may be fixing code from before the boundary."))
        else:
            print(_("No more candidates found."))
            _reset_suggest_fixup_state()
        sys.exit(1)
...
def command_suggest_fixup_line(line_id_specification: str, boundary: str | None = None, reset: bool = False, abort: bool = False, show_last: bool = False) -> None:
