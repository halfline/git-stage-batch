...
_ALLOWED_NEXT_ACTIONS = {
    ...
    HistoryPhase.FAILED: frozenset({HistoryNextAction.NONE}),
}
_ALLOWED_TRANSITIONS = {
    ...
    HistoryPhase.FAILED: frozenset({HistoryPhase.ABORTED}),
}
...
def active_history_operation_id() -> str | None:
    ...
    try:
        ...
        _invalid(f"cannot inspect active operation: {error}")
    if not stat.S_ISREG(metadata.st_mode):
        _invalid("active must be a regular file")
    try:
        operation_id = path.read_text(encoding="ascii").strip()
    except (OSError, UnicodeError) as error:
        _invalid(f"cannot read active operation: {error}")
    _validate_operation_id(operation_id)
    return operation_id
...
def _validate_state(state: HistoryOperationState) -> None:
    if state.schema_version != CURRENT_HISTORY_STATE_SCHEMA_VERSION:
        _invalid(
            f"schema_version must be {CURRENT_HISTORY_STATE_SCHEMA_VERSION}"
        )
    ...
    for index, refname in enumerate(state.allowed_remote_refs):
        if not refname.startswith("refs/remotes/") or "\0" in refname:
            _invalid(
                f"allowed_remote_refs[{index}] must be a full remote-tracking ref"
            )
...
def load_active_history_operation() -> HistoryOperationState | None:
    ...
    if state.operation_id != operation_id:
        _invalid("active pointer and state operation_id disagree")
    return state
...
def update_history_operation(state: HistoryOperationState) -> None:
    ...
    expected_output_tip = (
        state.output_commits[-1] if state.output_commits else None
    )
    if (
        _ref_is_symbolic(state.output_ref)
        or _resolved_object(state.output_ref) != expected_output_tip
    ):
...
def _persisted_plan_matches_state(state: HistoryOperationState) -> bool:
    plan_path = _require_operation_directory(state.operation_id) / "plan.json"
    if _file_sha256(plan_path) != state.plan_sha256:
        return False
    try:
        document = require_object(
            loads(read_required_text_file_contents(plan_path)),
            "document",
        )
...
def inspect_history_operation(
    ...
    output_ref_matches = (
        ...
        and _resolved_object(state.output_ref) == expected_output_tip
    )
    if (
        output_objects_exist
        and state.last_verified_commit is not None
        and _resolved_tree(state.last_verified_commit)
        != state.last_verified_tree
    ):
        output_objects_exist = False

    branch_ref_matches = current_branch == state.branch_ref
    branch_tip_matches = current_tip == state.expected_branch_tip
    ...
    if safety.active_history_operation != state.operation_id:
