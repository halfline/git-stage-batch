...
def get_start_head_file_path() -> Path:
    """Get path to file storing HEAD SHA at session start."""
    return get_state_directory_path() / "start-head"


def get_start_index_tree_file_path() -> Path:
    """Get path to file storing index tree SHA at session start."""
...
def get_hunk_counts() -> dict[str, int]:
    ...
    Returns:
        ...
        - remaining: Number of hunks still pending (approximation based on current state)
    """
...
def get_file_progress() -> tuple[int, int]:
    ...
    Returns:
        Tuple of (current_file_index, total_files)
        Returns (0, 0) if no current file is cached
    """
    # Read current lines to get the file path
    current_lines_file = get_current_lines_json_file_path()
    if not current_lines_file.exists():
        return (0, 0)

    try:
        content = read_text_file_contents(current_lines_file)
        ...
        if current_path in files:
            # 1-based index
            current_index = files.index(current_path) + 1
