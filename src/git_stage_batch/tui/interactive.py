...
def _handle_from(flow_state: FlowState) -> None:
    ...
    # Build menu
    options = []
    current_marker = _(" (current)")

    # Option 1: Working tree
    is_current = flow_state.source.role is LocationRole.WORKING_TREE
    marker = current_marker if is_current else ""
    text = _("Working tree{marker}").format(marker=marker)
    if use_color and is_current:
    ...
    for idx, name in enumerate(batches, 2):
        metadata = read_batch_metadata(name)
        note = metadata.get("note", "")
        is_current = flow_state.source.role is LocationRole.BATCH and flow_state.source.batch_name == name
        marker = current_marker if is_current else ""
        note_display = f" - {note}" if note else ""
...
def _handle_to(flow_state: FlowState) -> None:
    ...
    # Option 1: Staging
    is_current = flow_state.target.role is LocationRole.STAGING_AREA
    marker = current_marker if is_current else ""
    ...
    for idx, name in enumerate(batches, 2):
        metadata = read_batch_metadata(name)
        note = metadata.get("note", "")
        is_current = flow_state.target.role is LocationRole.BATCH and flow_state.target.batch_name == name
        marker = current_marker if is_current else ""
        note_display = f" - {note}" if note else ""
        text = _("batch: {name}{note}{marker}").format(
            name=name,
