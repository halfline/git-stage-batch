...
from ..i18n import _
from ..output import Colors, print_annotated_hunk_with_aligned_gutter
...
def _handle_again() -> None:
    ...
    hunk_hash_file = get_current_hunk_hash_file_path()
...
def _handle_batch() -> None:
    ...
    print(f"  [c]reate  [e]dit  [d]rop  [a]pply")
    print()
...
def _batch_create() -> None:
    ...
    command_new_batch(batch_id=batch_id, note=note if note else None)
    print(_("\nBatch '{name}' created.").format(name=batch_id))


def _batch_edit() -> None:
    """Prompt to select batch and edit its note."""
    batch_name = _prompt_select_batch(purpose=_("edit"))
    if not batch_name:
        return
...
def _prompt_select_batch(purpose: str) -> str:
    ...
    for idx, (name, note) in enumerate(batches, 1):
        note_display = f" - {note}" if note else ""
        print(f"  [{idx}] batch: {name}{note_display}")
    ...
    if choice.isdigit():
        ...
        if 0 <= idx < len(batches):
            return batches[idx][0]  # batch name
...
def start_interactive_mode() -> None:
    ...
    while True:
        ...
        if should_refresh and current_lines is not None:
...
def handle_line_selection() -> None:
    ...
    changed_ids = [line.id for line in current_lines.lines if line.id is not None]
    if not changed_ids:
