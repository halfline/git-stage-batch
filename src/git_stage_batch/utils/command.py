...
This module provides true concurrent streaming of stdin, stdout, stderr, and
additional child file descriptors. The core is binary-only and chunk-based
(not line-based).
...
class _SpawnedProcess:
    ...
    def __init__(self, pid: int):
        ...
        # These are never used but kept for compatibility
        self.stdin = None
        self.stdout = None
        self.stderr = None
...
class StreamingProcess:
    ...
    def __init__(
        ...
        self._stdin_current_chunk: bytes | None = None
    ...
    def stream(self, stdin_chunks: Iterable[bytes] | None = None) -> Iterator[CommandEvent]:
        ...
        IMPORTANT: Iteration completes based on output fd closure, not process exit.
    ...
    def _update_stdin_registration(self) -> None:
        ...
        has_data = self._stdin_current_chunk is not None or self._stdin_iter is not None
    ...
    def _should_close_stdin_now(self) -> bool:
        return (
            ...
            and self._stdin_current_chunk is None
    ...
    def _handle_writable_stdin(self, parent_fd: int) -> StdinClosedEvent | None:
        ...
        while self._stdin_current_chunk is None and self._stdin_iter is not None:
            try:
                chunk = next(self._stdin_iter)
                if chunk:  # Skip empty chunks
                    self._stdin_current_chunk = chunk
                    self._stdin_chunk_offset = 0
        ...
        if self._stdin_current_chunk is None:
            return None

        # Write what we can
        view = self._stdin_current_chunk[self._stdin_chunk_offset:]
        ...
        if written > 0:
            ...
            if self._stdin_chunk_offset >= len(self._stdin_current_chunk):
                # Done with this chunk
                self._stdin_current_chunk = None
                self._stdin_chunk_offset = 0
        else:
