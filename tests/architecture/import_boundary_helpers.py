...
def module_name_for_path(path: Path) -> str:
    """Return the internal module name for a source path."""
...
def resolve_import_from_module(
    *,
    ...
    """Resolve one import-from node relative to its containing module."""
    return _resolve_import_from_module(
...
def import_from_nodes(path: Path) -> list[tuple[str | None, ast.ImportFrom]]:
    """Return resolved import-from nodes in one source file."""
...
def package_path_for_module(module: str) -> Path | None:
    """Return a package directory for an internal module when one exists."""
...
def external_package_child_module_import_violations(
    ...
    """Return external imports of child modules reserved for package internals."""
    violations = []
    ...
    for path in SRC_ROOT.rglob("*.py"):
        for imported_module, node in import_from_nodes(path):
            ...
            disallowed_names = imported_names & disallowed_children[imported_module]
            if disallowed_names:
