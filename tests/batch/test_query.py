...
def test_list_batch_files_empty(temp_git_repo):
    """Test listing files in newly created batch starts with HEAD's tree."""
    create_batch("test-batch", "Test")

    files = list_batch_files("test-batch")
    # Newly created batch starts with HEAD's tree (contains README from fixture)
    assert files == ["README"]
...
def test_list_batch_files_sorted(temp_git_repo):
    ...
    assert files == ["README", "apple.txt", "middle.txt", "zebra.txt"]
