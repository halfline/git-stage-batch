...
"""
        ...
        # First patch is the empty file
        assert patches[0].old_path == "empty.txt"
        assert patches[0].new_path == "empty.txt"

        # Second patch is the normal file
...
class TestBinaryFiles:
    ...
    def test_binary_file_followed_by_text_file(self):
        """Binary file should be parsed as BinaryFileChange and not block text files."""
        diff = b"""\
...
"""
        ...
        assert patches[0].new_path == "image.png"
...
"""
        ...
        assert patches[1].change_type == "modified"
        assert isinstance(patches[2], SingleHunkPatch)
        assert patches[2].new_path == "after.txt"

    def test_binary_file_added(self):
        """Test parsing newly added binary file."""
        diff = b"""\
diff --git a/new_image.jpg b/new_image.jpg
new file mode 100644
...
"""
        ...
        assert len(patches) == 1
        assert isinstance(patches[0], BinaryFileChange)
        assert patches[0].old_path == "new_image.jpg"
        assert patches[0].new_path == "new_image.jpg"
        assert patches[0].change_type == "added"
    ...
    def test_binary_file_deleted(self):
        ...
        diff = b"""\
diff --git a/old_image.png b/old_image.png
deleted file mode 100644
...
"""
        ...
        assert len(patches) == 1
        assert isinstance(patches[0], BinaryFileChange)
        assert patches[0].old_path == "old_image.png"
        assert patches[0].new_path == "old_image.png"
