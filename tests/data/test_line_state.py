...
class TestConvertCurrentLinesToSerializableDict:
    ...
    def test_converts_current_lines_to_dict(self, sample_current_lines):
        """Test that CurrentLines is properly converted to a dictionary."""
        result = convert_current_lines_to_serializable_dict(sample_current_lines)
    ...
    def test_converts_line_entries(self, sample_current_lines):
    ...
    def test_result_is_json_serializable(self, sample_current_lines):
...
class TestLoadCurrentLinesFromState:
    ...
    def test_returns_none_when_no_state_exists(self, temp_git_repo):
        ...
        result = load_current_lines_from_state()
    ...
    def test_returns_none_when_only_json_exists(self, temp_git_repo):
        ...
        json_path = get_current_lines_json_file_path()
        json_path.write_text('{"path": "test.py"}')

        result = load_current_lines_from_state()
        assert result is None

    def test_loads_current_lines_from_state(self, temp_git_repo, sample_current_lines):
        """Test that CurrentLines is properly loaded from state files."""
        # Write state files
        patch_path = get_current_hunk_patch_file_path()
        json_path = get_current_lines_json_file_path()

        patch_path.write_text("dummy patch")
        serialized = convert_current_lines_to_serializable_dict(sample_current_lines)
    ...
    def test_loads_line_entries_correctly(self, temp_git_repo, sample_current_lines):
...
class TestComputeRemainingChangedLineIds:
    ...
    def test_errors_when_no_current_hunk(self, temp_git_repo):
        """Test that an error is raised when no current hunk exists."""
        with pytest.raises(CommandError) as exc_info:
            compute_remaining_changed_line_ids()
        assert "No current hunk" in str(exc_info.value.message)

    def test_returns_all_changed_ids_when_none_processed(self, temp_git_repo, sample_current_lines):
    ...
    def test_excludes_included_lines(self, temp_git_repo, sample_current_lines):
    ...
    def test_excludes_skipped_lines(self, temp_git_repo, sample_current_lines):
    ...
    def test_excludes_both_included_and_skipped_lines(self, temp_git_repo, sample_current_lines):
    ...
    def test_returns_sorted_ids(self, temp_git_repo):
        ...
        patch_path.write_text("dummy patch")
        serialized = convert_current_lines_to_serializable_dict(current_lines)
        write_text_file_contents(json_path, json.dumps(serialized))
