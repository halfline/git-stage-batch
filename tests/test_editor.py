...
class TestBuildTargetIndexContent:
    ...
    def test_no_trailing_newline(self):
        """Test handling of file without trailing newline."""
        header = HunkHeader(1, 1, 1, 2)
        lines = [
            LineEntry(None, " ", 1, 1, "line1"),
            LineEntry(1, "+", None, 2, "line2"),
        ...
        current_lines = CurrentLines(path="test.txt", header=header, lines=lines)
        base_text = "line1"  # No trailing newline

        result = build_target_index_content_with_selected_lines(current_lines, {1}, base_text)

        # Since base has no trailing newline and we have output, we add one
        assert result == "line1\nline2\n"
...
class TestBuildTargetWorkingTreeContent:
    ...
    def test_multiple_deletions_discarded(self):
        """Test discarding multiple deletions (reinserting them)."""
        header = HunkHeader(1, 4, 1, 1)
        lines = [
            LineEntry(1, "-", 1, None, "restore1"),
            LineEntry(2, "-", 2, None, "restore2"),
            LineEntry(3, "-", 3, None, "restore3"),
            LineEntry(None, " ", 4, 1, "kept"),
        ]
        current_lines = CurrentLines(path="test.txt", header=header, lines=lines)
        working_text = "kept\n"
...
class TestUpdateIndexWithBlobContent:
    ...
    def test_update_existing_file(self, temp_git_repo):
        ...
        (temp_git_repo / "test.txt").write_text("original\n")
        subprocess.run(["git", "add", "test.txt"], check=True, cwd=temp_git_repo, capture_output=True)
        subprocess.run(["git", "commit", "-m", "Initial"], check=True, cwd=temp_git_repo, capture_output=True)

        # Update index with modified content
        update_index_with_blob_content("test.txt", "modified\n")

        # Verify updated content in index
        result = subprocess.run(
            ["git", "show", ":test.txt"],
            check=True,
            cwd=temp_git_repo,
    ...
    def test_preserves_file_mode(self, temp_git_repo):
        ...
        (temp_git_repo / "script.sh").chmod(0o755)
    ...
    def test_default_mode_for_new_file(self, temp_git_repo):
        """Test that new files get default mode 100644."""
        ensure_state_directory_exists()

        # Create initial commit (needed for git commands to work)
    ...
    def test_temporary_blob_cleaned_up(self, temp_git_repo):
        """Test that temporary blob file is cleaned up."""
        ensure_state_directory_exists()
        ...
        temp_blob = get_state_directory_path() / ".temporary_index_blob"

        # Update index
        update_index_with_blob_content("test.txt", "new content\n")

        # Temporary blob should be cleaned up
        assert not temp_blob.exists()
